...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/09 16:19:40 by madecreu          #+#    #+#             */
/*   Updated: 2018/05/09 16:19:43 by madecreu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../incs/fract_ol.h"

void	calc_mandelbrot(t_frac frac, t_img img, t_point coor, double p_i)
{
	double	p_r;
	double	new_r;
	double	new_i;
	...
	{
		tmp = new_r;
		new_r = new_r * new_r - new_i * new_i + p_r;
		new_i = 2.0 * tmp * new_i + p_i;
		i++;
	}
	if (i == frac.i)
...
void	draw_mandelbrot(t_mlx mlx, t_img img, t_frac frac)
{
	...
	while (coor.y < img.h)
	{
		...
		while (coor.x < img.w)
		{
			calc_mandelbrot(frac, img, coor, p_i);
			coor.x++;
		}
		coor.y++;
	}
	mlx_put_image_to_window(mlx.mlx, mlx.win, img.ptr, 0, 0);
	mlx_string_put(mlx.mlx, mlx.win, 430, 0, 0XFFFFFF, "Mandelbrot");
	if (img.it == 1)
	{
