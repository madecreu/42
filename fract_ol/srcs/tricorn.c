...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/11 20:24:20 by madecreu          #+#    #+#             */
/*   Updated: 2018/05/11 20:24:22 by madecreu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../incs/fract_ol.h"

void	calc_tricorn(t_frac frac, t_img img, t_point coor, double p_i)
{
	double	p_r;
	double	new_r;
	double	new_i;
...
void	draw_tricorn(t_mlx mlx, t_img img, t_frac frac)
{
	...
	while (coor.y < img.h)
	{
		...
		while (coor.x < img.w)
		{
			calc_tricorn(frac, img, coor, p_i);
			coor.x++;
		}
		coor.y++;
	}
	mlx_put_image_to_window(mlx.mlx, mlx.win, img.ptr, 0, 0);
	mlx_string_put(mlx.mlx, mlx.win, 445, 0, 0XFFFFFF, "Tricorn");
	if (img.it == 1)
	{
